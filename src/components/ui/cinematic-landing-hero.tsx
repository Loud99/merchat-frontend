...
export function CinematicHero({
  brandName = "merchat.io",
  tagline1 = "Run your entire WhatsApp",
  tagline2 = "business on autopilot.",
  cardHeading = "AI commerce, redefined.",
  cardDescription = (
    <>
      <span className="text-white font-semibold">Merchat</span> gives Nigerian merchants an AI that
      talks to customers, closes orders, and manages inventory — all inside WhatsApp.
    </>
  ),
  metricValue = 500,
  metricLabel = "Orders Automated",
  ctaHeading = "Your AI is ready.",
  ctaDescription = "Join merchants across Nigeria who have automated their WhatsApp sales. Setup takes 10 minutes.",
  className,
  ...props
}: CinematicHeroProps) {
  ...
  // Cinematic autoplay timeline — no scroll required
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Phase 1: intro text reveal (runs immediately)
      const introTl = gsap.timeline({ delay: 0.3 });
      introTl
        .to(".text-track", { duration: 1.8, autoAlpha: 1, y: 0, scale: 1, filter: "blur(0px)", rotationX: 0, ease: "expo.out" })
        .to(".text-days", { duration: 1.4, clipPath: "inset(0 0% 0 0)", ease: "power4.inOut" }, "-=1.0");

      // Phase 2: cinematic sequence — starts 1.2s after intro text finishes
      const tl = gsap.timeline({ delay: 2.5 });

      tl
        // Text blurs out, card rises from below (parallel)
        .to([".hero-text-wrapper", ".bg-grid-theme"], { scale: 1.15, filter: "blur(20px)", opacity: 0.2, ease: "power2.inOut", duration: 1.4 }, 0)
        ...
        // Phone mockup swoops in
        .fromTo(".mockup-scroll-wrapper",
          { y: 300, z: -500, rotationX: 50, rotationY: -30, autoAlpha: 0, scale: 0.6 },
          { y: 0, z: 0, rotationX: 0, rotationY: 0, autoAlpha: 1, scale: 1, ease: "expo.out", duration: 1.8 }, "-=0.6"
        )
        // Phone widgets appear with stagger
        .fromTo(".phone-widget", { y: 40, autoAlpha: 0, scale: 0.95 }, { y: 0, autoAlpha: 1, scale: 1, stagger: 0.12, ease: "back.out(1.2)", duration: 1.2 }, "-=1.2")
        // Progress ring and counter animate together
        .to(".progress-ring", { strokeDashoffset: 60, duration: 1.8, ease: "power3.inOut" }, "-=0.8")
        .to(".counter-val", { innerHTML: metricValue, snap: { innerHTML: 1 }, duration: 1.8, ease: "expo.out" }, "<")
        // Floating badges pop in
        .fromTo(".floating-badge", { y: 100, autoAlpha: 0, scale: 0.7, rotationZ: -10 }, { y: 0, autoAlpha: 1, scale: 1, rotationZ: 0, ease: "back.out(1.5)", duration: 1.2, stagger: 0.18 }, "-=1.4")
        // Card text slides in from both sides
        .fromTo(".card-left-text", { x: -50, autoAlpha: 0 }, { x: 0, autoAlpha: 1, ease: "power4.out", duration: 1.2 }, "-=1.0")
        .fromTo(".card-right-text", { x: 50, autoAlpha: 0, scale: 0.8 }, { x: 0, autoAlpha: 1, scale: 1, ease: "expo.out", duration: 1.2 }, "<")
        // Hold on card content
        .to({}, { duration: 2.0 })
        // Swap to CTA
        ...
        // Card pulls back to reveal CTA behind it
        .to(".main-card", {
        ...
        // Card flies off screen — sequence complete
  ...
  function handleScrollToNext(e: React.MouseEvent<HTMLAnchorElement>) {
    e.preventDefault();
              ...
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                ...
                >
                  ...
                  <div className="absolute inset-[7px] bg-[#050914] rounded-[2.5rem] overflow-hidden shadow-[inset_0_0_15px_rgba(0,0,0,1)] text-white z-10">
                    ...
                    {/* WhatsApp Commerce App Interface */}
                    <div className="relative w-full h-full pt-12 px-5 pb-8 flex flex-col">
                      <div className="phone-widget flex justify-between items-center mb-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-bold tracking-tight text-white drop-shadow-md">Orders</span>
                        </div>
                      ...
                      <div className="phone-widget relative w-44 h-44 mx-auto flex items-center justify-center mb-8 drop-shadow-[0_15px_25px_rgba(0,0,0,0.8)]">
                        <svg className="absolute inset-0 w-full h-full" aria-hidden="true">
                          ...
                          <circle className="progress-ring" cx="88" cy="88" r="64" fill="none" stroke="#D5652B" strokeWidth="12" />
                        </svg>
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">AI closed an order</p>
                    <p className="text-orange-200/50 text-[10px] lg:text-xs font-medium">₦18,500 — Ankara Dress</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Payment confirmed</p>
                    <p className="text-emerald-200/50 text-[10px] lg:text-xs font-medium">Ngozi — just now</p>
                  </div>
                </div>
              </div>
            </div>
